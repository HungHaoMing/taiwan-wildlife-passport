...
目前 `animal01`～`animal07` 只是為相容既有瀏覽器資料而保留的內部 ID，不代表作品主題。2026-09-15 已依專案擁有者提供的正式資料填入七件作品三語名稱、介紹、替代文字與八張作品介紹圖；磁吸式智慧電路教具使用兩張圖。等待專案擁有者確認預覽後，第一階段才算完成。
...
2026-09-15 已將 `eventConfig.event.defaultLanguage` 從 `en` 改為 `zh-TW`，等待專案擁有者確認預覽。驗證範圍：
