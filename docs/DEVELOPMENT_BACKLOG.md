...
- 前端 `main`：`527212c`（七件作品內容、介紹圖、4 × 6 卡面、預設繁中與 Service Worker v2 已合併；米餐具圖已更正）
- Server `main`：`14b0278`（過期 session 導回登入已合併；31 項測試通過；待 NAS 部署）

## 專案擁有者指定的開發順序

不得跳階段。每階段完成、預覽並經專案擁有者確認後，才能開始下一階段。

### 1. 先補齊七件作品內容與介紹照片（已完成）

目前 `animal01`～`animal07` 只是為相容既有瀏覽器資料而保留的內部 ID，不代表作品主題。2026-09-15 已依專案擁有者提供的正式資料填入七件作品三語名稱、介紹、替代文字與八張作品介紹圖；2026-09-16 已更正米餐具介紹圖。內容實作已完成，剩餘項目納入卡面與真機最終驗收。

已由專案擁有者提供：

- 每件作品與 `stamp-source-01.png`～`stamp-source-07.png` 的對應確認。
- 中文正式名稱與作品介紹；未提供的補充資訊及作者／團隊／單位欄位不保留。
- 英文、日文名稱與介紹。
...
2026-09-15 已將 `eventConfig.event.defaultLanguage` 從 `en` 改為 `zh-TW`；自動測試與 390 × 844 預覽已確認新使用者預設繁中，既有英文或日文設定仍會保留。驗證範圍：
...
- **前端 Service Worker 更新策略（已實作，待正式升級驗收）**：2026-09-16 已升級為 `2026-tie-point-card-v2`，導覽請求採 network-first、離線才回退至快取入口，靜態檔保留 cache-first，並在 activate 清除舊 cache。不清除 `localStorage`。部署後需用曾開啟過 v1 的手機驗證自動更新。
- **Server 過期 session 導回登入（已實作，待 NAS 驗收）**：Server `14b0278` 已讓後台清單、詳情、列印頁與清單自動更新在 session 過期時導向 `/admin/login`；API、圖片與受保護程式檔仍維持 401。
- **卡面位置最終確認**：2026-09-16 已依 3600 × 2400 正式底圖重新量測七個白格中心並修正七枚印章座標；印章尺寸與順時鐘 14.9° 旋轉維持不變。姓名、手寫／繪圖及鍵盤文字仍需專案擁有者做最後預覽確認。
