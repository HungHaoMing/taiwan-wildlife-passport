...
2. 作品內容確認後，將新使用者預設語言由英文改成繁體中文，同時保留既有使用者自行選擇的語言。（已實作，待確認）
3. 最後才處理 Service Worker 舊快取、Server session 過期導回登入、低成本資安／維運改善及多人共用手機功能。

不要因為看見後文的「第三階段」就先開始多人功能；最新 backlog 是唯一執行順序來源。

目前狀態：七件作品的正式三語名稱、介紹與八張作品介紹圖已於 2026-09-15 加入，等待專案擁有者確認預覽；磁吸式智慧電路教具使用兩張圖。姓名、手寫／文字區與七枚印章百分比座標已實作。另已依新展場流程加入「5～7 枚完成後投稿總關 API 並顯示兌換碼」功能。不要重做已完成項目，也不要在確認前開始下一階段。
...
5. `eventConfig.animals` 已更新為 7 筆；此欄位名稱只為舊資料相容，不代表作品是動物主題。
6. 每一站使用不同且不可預測的 token；不要在回覆中完整顯示 token。
7. 搜尋並修正所有寫死的「6 枚／six／6個」文案與測試，完成條件一律以設定中的作品數量為準。
8. 確認首頁、集章卡、作品介紹、完成圖與輸出 PNG 都引用新版素材。
