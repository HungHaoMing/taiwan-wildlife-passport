...
- 五枚以上會顯示「前往手寫完成卡片」按鈕，第七枚才會自動進入；手寫支援高 DPI、觸控、復原、清除、重寫及重載復原。
...
- 活動方提供的新版來源底卡原檔保存在 `public/assets/event/2026-tie/point-card-source-v2.png`，尺寸 3590 × 2409，未改動。
- 正式底圖 `point-card-4x6.png` 與高解析輸出均為 3600 × 2400，即標準 3:2（4 × 6）比例；衍生方式記錄於 `docs/ASSET_PROVENANCE.md`。
- 七枚 RGBA 印章原檔保存在同一資料夾，顯示時順時鐘旋轉 14.9°；2026-09-16 已依 3600 × 2400 正式底圖重新量測七個白格中心並修正定位。
...
- 最終手機上的透明印章邊緣、三語字型與 3600 × 2400 圖片清晰度。
