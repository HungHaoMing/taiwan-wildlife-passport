...
- English 預設；日本語與繁體中文目前明確回退到簡單英文 placeholder，補翻譯只需改設定檔。
...
- 一般網址：部署根網址
- QR 管理頁：`部署根網址?view=qr`
- 工作人員模式：一般頁按 **Staff mode**，或開啟 `部署根網址?staff=1`
- 預設工作人員 PIN：`2468`，請在 `staff.pin` 修改。

QR 管理頁會依「當下部署網址 + 子路徑」產生六個正確連結，可複製、逐張下載或直接列印。建議部署後才輸出正式 QR Code。現場先用每一張 QR Code 實掃一次，確認站名和印章一致。
