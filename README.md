...
供「2026TIE台灣創新技術博覽會－暨大與暨大附中」使用的手機優先靜態網站。參加者不需安裝 App 或登入；姓名、印章、留言與完成狀態只存在使用者目前瀏覽器的 `localStorage`，不會上傳伺服器。
...
- 第七章完成後自動進入手寫流程；支援觸控、高 DPI、旋轉後保留、復原、清除與鍵盤文字。
...
`npm test` 會實際驗證姓名／留言限制、ID + token 驗證、亂序七章、重複章、完成狀態、本機保存重載、污染資料正規化、進度匯出／匯入，以及七組素材與百分比座標設定。`npm run build` 會驗證正式 bundle 與相對資源輸出。每次 GitHub Pages 部署也會重跑兩者。
